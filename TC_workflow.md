...
2. `vmc_opt` (mandatory)
	...
	- The `gjastrow` form of CASINO can be used to generate different types of Jastrow factor. The commonly-used one is Drummond–Towler–Needs (DTN) factor, which can be expanded by `natural power` basis function:$$\phi_k(r)=r^{k-1}$$
     ...
     The Jastrow factor is written as:$$J(r)=e^{[\sum_k c_k \phi_k(r)]\times f(r)}$$
     Therefore, the e-e cusp condition $\lim_{r_{ij}\rightarrow 0} \frac{\partial u(r_{ij})}{\partial r_{ij}}=1/2$ will result in:$$c_1 \times(-\frac{C}{L})+ c_2=1/2 \rightarrow c_1=(c_2-1/2)\times\frac{L}{C}$$; the same, the e-n cusp condition $\lim_{r_{Ii}\rightarrow 0} \frac{\partial u(r_{Ii})}{\partial r_{Ii}}=-Z$ result in:$$c_1=(c_2+Z)\times\frac{L}{C}.$$
